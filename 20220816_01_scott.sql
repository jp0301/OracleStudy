...
-- 날짜, 숫자, 문자타입에 사용이됬는데 특이사항이 있다.
-- 사전식 배열 기준
-- S라는 이름이면 나오겠지만 만약 SA, SAB, SB... 등은 안나온다.
-- S까지 나온다.

--※ BETWEEN ⓐ AND ⓑ 는 날짜형, 숫자형, 문자형 데이터 모두에 적용된다.
--   단, 문자형일 경우 아스키 코드 순서를 따르기 때문에 (사전식 배열)
...
-- OR, IN, =ANY 3가지로쓸 수 있지만 실제로 오라클은 OR로 처리한다.
...
-- DROP TABLE TBL_SAWON PURGE; -- 휴지통을거치지 않고 바로 삭제
-- 쓸일 없으니 알아만 둘 것
...
SELECT SANO, SANAME, SAL
FROM  TBL_SAWON
WHERE SANAME LIKE '_보_'; --전체 글자가 3개인 것 중에만이라는 조건이 붙음
--==>>
/*
1002	김보경	2000
1014	반보영	5200
...
--> EMP 테이블이 갖고 있는 테이블의 고유한 컬럼(2→NAME)가 아니라
-- SELECT 처리되는 두 번째 컬럼(2→DEPTNO, 부서번호)을 기준으로 정렬
...
--○ CONCAT()
SELECT ENAME || JOB "첫번째컬럼", CONCAT(ENAME, JOB) "두번째 컬럼"
FROM EMP;
...
--○ SUBSTR() 갯수 기반 / SUBSTRB() 바이트 기반(항상 인코딩 조심) 가급적 사용 제한

SELECT ENAME "COL1"
    ,  SUBSTR(ENAME, 1, 2) "COL2" --1번째부터 2개.
FROM EMP;
--> 문자열을 추출하는 기능을 가진 함수
